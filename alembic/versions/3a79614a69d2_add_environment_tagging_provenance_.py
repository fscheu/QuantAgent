...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_start_end_date", "backtest_runs", ["start_date", "end_date"], unique=False
    )
    op.create_index(
        op.f("ix_backtest_runs_created_at"),
        "backtest_runs",
        ["created_at"],
        unique=False,
    )
    op.create_index(
        op.f("ix_backtest_runs_end_date"), "backtest_runs", ["end_date"], unique=False
    )
    op.create_index(
        op.f("ix_backtest_runs_start_date"),
        "backtest_runs",
        ["start_date"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_strategy_configs_created_at"),
        "strategy_configs",
        ["created_at"],
        unique=False,
    )
    op.create_index(
        op.f("ix_strategy_configs_id"), "strategy_configs", ["id"], unique=False
    )
    op.create_index(
        op.f("ix_strategy_configs_name"), "strategy_configs", ["name"], unique=True
    )
    op.add_column(
        "orders",
        sa.Column(
            "environment",
            sa.Enum("BACKTEST", "PAPER", "PROD", name="environment"),
            nullable=False,
    ...
    op.add_column(
        "signals", sa.Column("thread_id", sa.String(length=100), nullable=True)
    )
    op.add_column(
        "signals", sa.Column("checkpoint_id", sa.String(length=100), nullable=True)
    )
    op.add_column("signals", sa.Column("state_snapshot", sa.JSON(), nullable=True))
    op.add_column(
        "signals", sa.Column("model_provider", sa.String(length=50), nullable=True)
    )
    op.add_column(
        "signals", sa.Column("model_name", sa.String(length=100), nullable=True)
    )
    op.add_column("signals", sa.Column("temperature", sa.Float(), nullable=True))
    op.add_column(
        "signals", sa.Column("agent_version", sa.String(length=50), nullable=True)
    )
    op.add_column(
        "signals", sa.Column("graph_version", sa.String(length=50), nullable=True)
    )
