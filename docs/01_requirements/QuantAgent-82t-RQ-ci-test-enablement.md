...
**Blocked by:** QuantAgent-sfc, QuantAgent-4ch
...
Currently, the "Run unit tests" step in `main-ci-deploy.yml` is disabled (lines 62-69):
```yaml
...
With **QuantAgent-sfc** (SQLite/JSONB fixes) and **QuantAgent-4ch** (test database configuration) resolved, the test suite should be clean and ready to run in CI.
...
- Update pytest command to use correct markers and settings
...
### Out of Scope
- Modifying test files or test logic (tests are already fixed by QuantAgent-4ch)
...
- Adding code coverage reporting (future enhancement)
...
```
(Note: `maxfail=10` instead of original `maxfail=5` to see more failures)
...
**When** tests fail  
**Then**:
- `continue-on-error: true` is **NOT** present (removed)
...
- **No test modifications**: Tests themselves are not changed (fixed by QuantAgent-4ch)
...
- **QuantAgent-sfc**: Fix SQLite/JSONB compatibility issues
- **QuantAgent-4ch**: Fix test database configuration (SQLite for local, PostgreSQL for CI)
...
- [ ] Correct pytest command configured
