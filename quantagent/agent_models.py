...
class IndicatorReport(BaseModel):
    ...
    macd: float = Field(
        description="MACD value (difference between 26-period and 12-period EMA)"
    )
    ...
    rsi_level: str = Field(
        description="RSI interpretation: 'overbought' (>70), 'oversold' (<30), or 'neutral'"
    )

    roc: float = Field(description="Rate of Change percentage")

    stochastic: float = Field(
        ge=0.0, le=100.0, description="Stochastic oscillator (0-100)"
    )

    willr: float = Field(ge=-100.0, le=0.0, description="Williams %R (-100 to 0)")

    trend_direction: str = Field(
        description="Interpreted trend: 'bullish', 'bearish', or 'neutral'"
    )
    confidence: float = Field(ge=0.0, le=1.0, description="Confidence level (0.0-1.0)")

    reasoning: str = Field(
        description="LLM analysis reasoning for indicator interpretation"
    )
...
class PatternReport(BaseModel):
    ...
    primary_pattern: Optional[str] = Field(
        default=None, description="Most confident pattern identified, if any"
    )

    confidence: float = Field(
        ge=0.0, le=1.0, description="Pattern confidence level (0.0-1.0)"
    )

    breakout_probability: float = Field(
        ge=0.0, le=1.0, description="Probability of breakout based on pattern (0.0-1.0)"
    )
...
class TradingDecision(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0, le=1.0, description="Overall decision confidence (0.0-1.0)"
    )
    ...
    entry_price: Optional[float] = Field(
        default=None, description="Suggested entry price if LONG/SHORT"
    )
