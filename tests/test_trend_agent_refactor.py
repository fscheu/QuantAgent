...
class TestOutputStructure:
    ...
    def test_output_is_trend_report_instance(
        ...
        assert isinstance(
            report, TrendReport
        ), "Output must be TrendReport Pydantic model"
...
class TestConstraintValidation:
    ...
    def test_trend_strength_within_0_to_1(
        self, mock_llm, mock_vision_llm, mock_toolkit, sample_state
    ):
        ...
        ), f"Trend strength out of range: {report.trend_strength}"

    def test_support_and_resistance_are_floats(
        self, mock_llm, mock_vision_llm, mock_toolkit, sample_state
    ):
    ...
    def test_support_typically_below_resistance(
        self, mock_llm, mock_vision_llm, mock_toolkit, sample_state
    ):
        ...
        if report.support_level > 0 and report.resistance_level > 0:
            assert (
                report.support_level <= report.resistance_level
...
class TestEdgeCases:
    ...
    def test_large_support_resistance_values(
        self, mock_llm, mock_vision_llm, mock_toolkit
    ):
