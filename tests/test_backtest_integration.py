...
class TestBacktestIntegration:
    ...
    def test_full_backtest_flow_minimal(
    ...
    def test_backtest_with_trades_execution(
        self, db_session, mock_market_data, sample_config
    ):
        """
        ...
        with patch("quantagent.backtesting.backtest.TradingGraph") as mock_tg_class:
            ...
            signals = (
                ...
                .filter(Signal.environment == Environment.BACKTEST)
                .all()
            )
    ...
    def test_backtest_metrics_accuracy(
        self, db_session, mock_market_data, sample_config
    ):
        """
    ...
    def test_backtest_handles_risk_rejections(
        self, db_session, mock_market_data, sample_config
    ):
        """
    ...
    def test_backtest_config_snapshot_reproducibility(
        self, db_session, mock_market_data, sample_config
    ):
        """
        ...
        with patch("quantagent.backtesting.backtest.TradingGraph") as mock_tg_class:
            ...
            run1 = (
                db_session.query(BacktestRun)
                .filter(BacktestRun.id == backtest1.backtest_run_id)
                .first()
            )

            run2 = (
                db_session.query(BacktestRun)
                .filter(BacktestRun.id == backtest2.backtest_run_id)
                .first()
            )
    ...
    def test_backtest_date_range_iteration(
        self, db_session, mock_market_data, sample_config
    ):
        """
    ...
    def test_backtest_equity_curve_tracking(
        self, db_session, mock_market_data, sample_config
    ):
        """
